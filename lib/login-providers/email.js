...
class EmailLoginProvider extends LoginProviderBase {
  ...
  login (
    ...
    return this._verifyEmail(email)
      ...
      .then(verifiedEmail => super.promptForCode('Please check your email and enter the verification code: ', verifiedEmail, 'email', storeJwt))
  }
        ...
        if (body.error) {
          ...
          switch (body.error_description) {
            case 'Connection does not exist': {
              return reject(new Error('This BlinkMobile service does not provide email driven passwordless authentication. Please use another type of authentication to login.'))
            }
            default: {
  ...
  _verifyEmail (
    ...
    return inquirer.prompt([{
      ...
      if (!results.email) {
        return Promise.reject(new Error('Please specify an email address to send verification code to.'))
      }
