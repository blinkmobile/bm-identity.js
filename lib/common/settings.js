 ...
 * @param {String} bmProject - The name of the project.
 * @returns {Object} The settings.
 */
function settings (clientName, bmProject) {
  return getJwt()
    .then((jwt) => {
      if (!jwt) {
      ...
      let url = `${decoded.serviceSettingsUrl}?bmService=${clientName}`;

      if (bmProject) {
        url += `&bmProject=${bmProject}`;
      }
