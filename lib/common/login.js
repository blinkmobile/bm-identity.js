...
function login (clientName, options) {
  ...
  return auth0ClientFactory.getClientIdByName(clientName).then(clientId => {
    if (options.username) {
      ...
      return loginProvider.login(options.username === true ? null : options.username, options.password);
    } else if (options.email) {
      const loginProvider = new EmailLoginProvider(clientId);
      return loginProvider.login(options.email === true ? null : options.email);
    } else if (options.sms) {
      const loginProvider = new SmsLoginProvider(clientId);
      return loginProvider.login(options.sms === true ? null : options.sms);
    } else {
